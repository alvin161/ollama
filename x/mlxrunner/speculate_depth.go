...
// double the interval up to this cap, dropping the probe rate to one per 512 rounds.
// That trades slower re-engagement — worst case plain decode speed, never below — for
// fewer probes.
...
func (c *depthController) next() int {
	...
	if seed := c.unsampledDepthInWindow(); seed >= 0 {
		return seed
	}
	if !c.cost.ready() {
		return 0
...
// unsampledDepthInWindow is the shallowest depth in [0, frontier+1] with no clean
// cost sample, or -1 if all are sampled; bounding to frontier+1 keeps cost-seeding
// from outrunning the acceptance frontier.
func (c *depthController) unsampledDepthInWindow() int {
	limit := c.frontier() + 1
	for n := 0; n <= limit; n++ {
		if !c.cost.sampled(n) {
			return n
...
func (c *depthController) selected() int {
	...
	limit := c.frontier() + 1
	best, bestEV := 0, c.committed(0)/c.cost.cost(0)
	for n := 1; n <= limit; n++ {
		if ev := c.committed(n) / c.cost.cost(n); ev > bestEV {
			best, bestEV = n, ev
		}
	}
	return best
...
func (m *costModel) cost(drafts int) float64 {
	return m.interp(drafts)
}

// interp piecewise-linearly interpolates the per-depth EWMAs, clamping to the
// nearest sample outside the sampled range (the curve beyond is unknown).
func (m *costModel) interp(drafts int) float64 {
	ds := m.depths
	if len(ds) == 0 {
		return 0
	}
