...
// open returns the speculation cursor for this request, or nil when there is
// nothing to draft and no drafter state to maintain. A logprobs request (cannot
// speculate) still streams committed runs to a drafter with draft caches to
// keep its draft KV prefix-cached. A nil receiver (no draft head) decodes plainly.
func (s *speculation) open(request Request, caches []cache.Cache) *speculationSession {
	...
	// only to maintain a draft cache in lockstep (permanently parked); a
	// draftless head has nothing to maintain and decodes plainly.
	opts := request.SamplerOpts
	enabled := !opts.Logprobs && opts.TopLogprobs == 0
	if !enabled && len(s.draftKV) == 0 {
		return nil
	}

	spec := &speculationSession{spec: s, drafter: d, enabled: enabled, prevDrafts: -1, roundDrafts: -1}
	if enabled {
		spec.limit = s.depth.next()
